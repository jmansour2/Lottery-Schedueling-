...
//Once you have the basics working, consider filling in some of the other system call stubs left for future expansion, or write a version of user_printf() that processes all of the format string in the user process,
// but relies on the user_putc() syscall for requesting that the O/S output each resulting character.
#include <xinu.h>
#include <stdio.h>
...
void infiniteloop(void){
	...
	for(;;){
		kprintf("This is current process %d\r\n,",currpid);
	}
}
void testcases(void)
{
    ...
    case '3':
        ...
        ready(create((void *)testmain, INITSTK,10, "MAIN1", 2, 0, NULL),
              RESCHED_NO);
        ready(create((void *)testmain, INITSTK,10, "MAIN2", 2, 0, NULL),
              RESCHED_NO);
        ready(create((void *)testmain, INITSTK,10, "MAIN3", 2, 0, NULL),
              RESCHED_YES);
    ...
    case 'p':
	ready(create((void *)infiniteloop, INITSTK, 10, "MAIN1", 2, 0, NULL),
              RESCHED_NO);
        ready(create((void *)infiniteloop, INITSTK, 10, "MAIN2", 2, 0, NULL),
              RESCHED_NO);
	break;
   ...
   default:
	...
	//create a new method to infinitly loop
	//create and call 2 or more versions of that method 
    kprintf("\r\n===TEST END===\r\n");
    return;
}
